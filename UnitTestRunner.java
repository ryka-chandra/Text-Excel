...
public class UnitTestRunner {
     ...
     * @param cmdHandler Callback to processCommand.
     * @return true if the command was a Test command. This means that the caller
     *         should ignore the input. False means the caller should proceed to
     *         process the command.
     */
    ...
    /**
     * Checks if input is "test create #"
     * <p>
     * 
     * If the input is of that form, it will redirect output to a file. When done
     * with the tests, the user should enter, "test end"
     * 
     * @param input The command string that the user entered to be processed.
     ...
     */
    private static double runTests(Scanner file, boolean breakOnFail, ArrayList<String> summary,
        ...
        while (file.hasNextLine()) {
            try {
                ...
                if (input.toLowerCase().startsWith("<timeout")) {
                    ...
                    // // subtotal
                    //
                    // Timeouts work for subsections only!!
                    ...
                    System.out.println("Resuming in main thread");
                    continue;
                }
                if (input.equalsIgnoreCase("// subtotal")) {
                    if (summary == null) {
                        // we are running in a thread (should be). Return the points and end this
                        // thread.
                        return subSectionPoints;
                    }
                ...
                String[] lineSplit = lineAndPts.split("\\s+");
                ...
                if (answerCount == 0 || multiLineMatch(actualFull, expected)) {
                    ...
                    if (answerCount == 0 && actualFull != null && actualFull.length() > 0) {
                        // There is a result, but it is ignored.
                ...
                System.out.println(" Failed due to a StackOverflow!!");
                if (breakOnFail) {
                    return total;
                }
            }
        ...
        if (summary != null) {
            String border = "**************************\n";
            String s = String.format("%sTOTAL SCORE: %.1f / %.1f\n%s\n", border, points, total, border);
            if (summary.size() == 0) {
                System.out.print(s);
            }
            summary.add(s);
    ...
    private static double startTimeout(Scanner file, int timeout, boolean breakOnFail, CommandHandler cmdHandler) {
        ...
        // Read the file up through "// subtotal" so that we can keep track of where
        // the main thread should start again even after a timeout.
        ...
        Scanner sectionScanner = new Scanner(section.toString());

        System.out.println("Starting thread with a timeout = " + timeout);
        double score = 0;
        ExecutorService executor = Executors.newSingleThreadExecutor();
        Future<Double> future = executor.submit(() -> {
            // pass in the Scanner that was given the Strings pre-read from the file
            return runTests(sectionScanner, breakOnFail, null, cmdHandler);
        });
        ...
        try {
            ...
            System.out.println("Finished! with score: " + score);
